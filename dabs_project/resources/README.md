...
The databricks CLI bundles top level is defined by file `dabs_project/databricks.yml`.
...
ML Resource Configurations in this directory:
 - model workflow (`dabs_project/resources/model-workflow-resource.yml`)
 - batch inference workflow (`dabs_project/resources/batch-inference-workflow-resource.yml`)
 - monitoring resource and workflow (`dabs_project/resources/monitoring-resource.yml`)
 - feature engineering workflow (`dabs_project/resources/feature-engineering-workflow-resource.yml`)
 - model definition and experiment definition (`dabs_project/resources/ml-artifacts-resource.yml`)
...
|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------|-------------------------------------|------------------------------------------------|
| dev         | The `dev` deployment target is used by ML engineers to deploy ML resources to development workspace with `dev` configs. The config is for ML project development purposes.                                                           | dev workspace        | dev-dabs_project-model     | /dev-dabs_project-experiment     |
| staging     | The `staging` deployment target is part of the CD pipeline. Latest main content will be deployed to staging workspace with `staging` config.                                                             | staging workspace    | staging-dabs_project-model | /staging-dabs_project-experiment |
| prod        | The `prod` deployment target is part of the CD pipeline. Latest release content will be deployed to prod workspace with `prod` config.                                                                      | prod workspace       | prod-dabs_project-model    | /prod-dabs_project-experiment    |
| test        | The `test` deployment target is part of the CI pipeline. For changes targeting the main branch, upon making a PR, an integration test will be triggered and ML resources deployed to the staging workspace defined under `test` deployment target. | staging workspace    | test-dabs_project-model    | /test-dabs_project-experiment    |
...
2. Complete the `TODO` in `dabs_project/databricks.yml` to add the dev workspace URI under `targets.dev.workspace.host`.
...
Following this, update the batch_inference_job base parameters in `dabs_project/resources/batch-inference-workflow-resource.yml` to pass
...
To get started, open `dabs_project/resources/batch-inference-workflow-resource.yml`.  The file contains the ML resource definition of a batch inference job, like:
...
resources:
  jobs:
    batch_inference_job:
      name: ${bundle.target}-dabs_project-batch-inference-job
      tasks:
...
The example above defines a Databricks job with name `${bundle.target}-dabs_project-batch-inference-job`
that runs the notebook under `dabs_project/deployment/batch_inference/notebooks/BatchInference.py` to regularly apply your ML model for batch inference. 
...
We specify a `batch_inference_job` under `resources/jobs` to define a databricks workflow with internal key `batch_inference_job` and job name `{bundle.target}-dabs_project-batch-inference-job`.
...
`test-dabs_project-batch-inference-job`. During the deployment of the `staging` environment config, the job name will be
`staging-dabs_project-batch-inference-job`.
